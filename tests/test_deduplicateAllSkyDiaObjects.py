...
class TestDeduplicateAllSkyDiaObjects(unittest.TestCase):
    """Tests for the full DeduplicateAllSkyDiaObjectsTask including
    run(), remap_clusters(), and count_duplicates().
    """

    def setUp(self):
        self.task = _make_task(maxClusteringDistance=1.5)

    def _make_apdb_with_duplicates(self):
        """Set up an APDB with known duplicate DiaObjects and return
        the task, expected duplicate IDs, and APDB access info.
        """
        rng = np.random.default_rng(1234)

        db_file_fd, db_file = tempfile.mkstemp(dir=os.path.dirname(__file__))
        self.addCleanup(os.remove, db_file)
        self.addCleanup(os.close, db_file_fd)

        apdbConfig = ApdbSql.init_database(db_url="sqlite:///" + db_file)
        config_file = tempfile.NamedTemporaryFile()
        self.addCleanup(config_file.close)
        apdbConfig.save(config_file.name)
        apdb = Apdb.from_config(apdbConfig)

        exposure = makeExposure(False, False)
        regionTime = makeRegionTime(exposure=exposure)
        dateTime = getMidpointFromTimespan(regionTime.timespan)

        original_diaObjects = makeDiaObjects(50, exposure, rng)

        duplicate_rows = []
        offsets = [
            (0, [(0, 0.2), (0.1, 0.2), (0, -0.1), (0.2, -0.1)], 1000),
            (1, [(0, 0.2), (0.1, 0.2), (0, -0.1)], 2000),
            (2, [(0, 0.2)], 3000),
        ]
        for obj_idx, offset_list, base_id in offsets:
            base_ra = original_diaObjects.iloc[obj_idx]['ra']
            base_dec = original_diaObjects.iloc[obj_idx]['dec']
            for i, (dra, ddec) in enumerate(offset_list):
                row = {"ra": base_ra + dra / 3600,
                       "dec": base_dec + ddec / 3600,
                       "diaObjectId": base_id + i,
                       "nDiaSources": 1}
                for f in ["u", "g", "r", "i", "z", "y"]:
                    row["%s_psfFluxNdata" % f] = 0
                duplicate_rows.append(row)

        duplicate_diaObjects = pd.DataFrame(
            data=duplicate_rows).set_index("diaObjectId", drop=False)
        diaObjects = pd.concat([original_diaObjects, duplicate_diaObjects])

        diaSources = makeDiaSources(
            100, diaObjects["diaObjectId"].to_numpy(), exposure, rng)
        diaForcedSources = makeDiaForcedSources(
            200, diaObjects["diaObjectId"].to_numpy(), exposure, rng)

        storeDateTime = regionTime.timespan.begin.tai - 30 * astropy.units.day
        apdb.store(storeDateTime, diaObjects, diaSources, diaForcedSources)

        config = DeduplicateAllSkyDiaObjectsTask.ConfigClass()
        config.apdb_config_url = config_file.name
        config.earliestMidpointMjdTai = dateTime.mjd
        task = DeduplicateAllSkyDiaObjectsTask(config=config)

        return task, apdb, regionTime, dateTime

    def testRun(self):
        """Test the full run method for deduplication.
        """
        task, apdb, regionTime, dateTime = self._make_apdb_with_duplicates()
        result = task.run()
        ...
        updated_diaSources = apdb.getDiaSources(
            region=regionTime.region, object_ids=None,
            visit_time=dateTime, start_time=None)
    ...
    def test_remap_clusters(self):
        ...
        dedup_map = self.task.remap_clusters(test_data)

        self.assertEqual(len(dedup_map), 3)
    ...
    def test_cluster(self):
        """Test clustering with known duplicate groups.
        """
        # Three groups of duplicates within 0.3 arcsec, well separated
        ra = [10.0, 10.0 + 0.1 / 3600, 10.0 + 0.2 / 3600,
              10.0, 10.0 + 0.2 / 3600,
              50.0, 50.0 + 0.1 / 3600, 50.0 + 0.2 / 3600,
              50.0,
              90.0, 90.0 + 0.2 / 3600]
        dec = [5.0, 5.0 + 0.2 / 3600, 5.0 - 0.1 / 3600,
               5.0 + 0.2 / 3600, 5.0 - 0.1 / 3600,
               20.0, 20.0 + 0.2 / 3600, 20.0 - 0.1 / 3600,
               20.0 + 0.2 / 3600,
               40.0, 40.0 + 0.2 / 3600]
        diaObjects = _make_diaObjects(ra, dec)
        cluster_labels = self.task.cluster(diaObjects)

        self.assertEqual(len(cluster_labels), len(diaObjects))

        # Group 1 (indices 0-4) should share a label
        group1_labels = cluster_labels.iloc[0:5]
        self.assertEqual(len(group1_labels.unique()), 1,
                         "Objects in duplicate group 1 should share a cluster label")

        # Group 2 (indices 5-8) should share a label
        group2_labels = cluster_labels.iloc[5:9]
        self.assertEqual(len(group2_labels.unique()), 1,
                         "Objects in duplicate group 2 should share a cluster label")

        # Group 3 (indices 9-10) should share a label
        group3_labels = cluster_labels.iloc[9:11]
    ...
    def test_count_duplicates(self):
        ...
        duplicate_count = self.task.count_duplicates(test_catalog)
        self.assertEqual(duplicate_count, 4)

        duplicate_count_small = self.task.count_duplicates(test_catalog, radius_arcsec=0.1)
        self.assertEqual(duplicate_count_small, 0)

        duplicate_count_large = self.task.count_duplicates(test_catalog, radius_arcsec=3600.0)
