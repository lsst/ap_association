...
"""Comprehensive unit tests for the `cluster` method of
DeduplicateAllSkyDiaObjectsTask.
"""
...
class TestClusterSmallDataset(unittest.TestCase):
    ...
    def test_contiguous_labels(self):
        """Cluster labels should be contiguous integers starting from 0."""
        # Create 10 well-separated objects
        ra = [float(i * 10) for i in range(10)]
        dec = [float(i * 10) for i in range(10)]
        diaObjects = _make_diaObjects(ra, dec)

        labels = self.task.cluster(diaObjects)

        unique_labels = sorted(labels.unique())
        self.assertEqual(unique_labels, list(range(len(unique_labels))))
...
class TestClusterEdgeCases(unittest.TestCase):
    ...
    def test_deterministic_results(self):
        """Calling cluster twice on the same data yields identical results."""
        rng = np.random.default_rng(42)
        n = 100
        ra = rng.uniform(0, 360, n).tolist()
        dec = rng.uniform(-90, 90, n).tolist()
        ...
        task = _make_task(maxClusteringDistance=1.5)
        labels1 = task.cluster(diaObjects)
        labels2 = task.cluster(diaObjects)
